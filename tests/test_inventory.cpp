...
int main() {
    ...
    { // AddOverflow (79 stacks cheias + 1 livre: sobra 151 de 250)
        Inventory inv;
        for (int i = 0; i < 79; ++i) assert(inv.add(Item{"stone", 99}) == 0);
        assert(inv.usedSlots() == 79);
        assert(inv.add(Item{"stone", 250}) == 151);
        assert(inv.count("stone") == 79 * 99 + 99);
